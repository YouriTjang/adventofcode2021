...
def parse_line(data):
    io = data.split(" | ")
    chunks = (io[0] + " " + io[1]).split(" ")
    chunks = ["".join(sorted(chunk)) for chunk in chunks]
    return chunks


def solve_lines(lines):
    solveds = list(map(lambda line: solve(line), lines))
    count = sum(solveds)
    return count
...
def solve(line):
    ...
    for chunk in chunks[:10]:
        if chunks_by_size.get(len(chunk)) is None:
            chunks_by_size[len(chunk)] = set()
        chunks_by_size[len(chunk)].add("".join(sorted(chunk)))

    mapping = get_mapping_part2(chunks_by_size)
    reverse_mapping = {v: k for k, v in mapping.items()}
    solution = ""
    for chunk in chunks[10:]:
        solution += str(reverse_mapping[chunk])
    s = len(solution)
    return int(solution)
...
def get_mapping_part2(chunks_by_size):
    part1 = {1: 2,
             4: 4,
             7: 3,
             8: 7, }
    mapping = {1: list(chunks_by_size.get(2))[0],
               4: list(chunks_by_size.get(4))[0],
               7: list(chunks_by_size.get(3))[0],
               8: list(chunks_by_size.get(7))[0]}
    chunks_by_size[2].discard(mapping[1])
    chunks_by_size[4].discard(mapping[4])
    chunks_by_size[3].discard(mapping[7])
    chunks_by_size[7].discard(mapping[8])

    calculate_chunk(chunks_by_size, 6, 6, 1, 1, mapping)
    calculate_chunk(chunks_by_size, 6, 9, 4, 0, mapping)
    get_remaining_chunk(chunks_by_size, 0, 6, mapping)

    calculate_chunk(chunks_by_size, 5, 3, 1, 0, mapping)
    calculate_chunk(chunks_by_size, 5, 5, 6, 1, mapping)
    get_remaining_chunk(chunks_by_size, 2, 5, mapping)

    return mapping


def calculate_chunk(chunks_by_size, size_bucket, digit, compare_to,
    expected_difference, mapping):
    for chunk in chunks_by_size[size_bucket]:
        difference = set(mapping.get(compare_to)).difference(set(chunk))
        if len(difference) == expected_difference:
            mapping[digit] = chunk
    chunks_by_size[size_bucket].discard(mapping[digit])


def get_remaining_chunk(chunks_by_size, digit, bucket, mapping):
    mapping[digit] = list(chunks_by_size[bucket])[0]
    chunks_by_size[bucket].discard(mapping[digit])


class MyTestCase(unittest.TestCase):
    def test_example(self):
        actual = solve_lines(example_data)
        self.assertEqual(43049, actual)

    def test_main_part1(self):
        actual = solve_lines(puzzle_data)
        self.assertEqual(412, actual)

    def test_example_part2(self):
        actual = solve_lines(example_data)
        self.assertEqual(61229, actual)

    def test_single_line_part2(self):
        actual = solve_lines([
                                 "be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe"])
        self.assertEqual(8394, actual)

    def test_main_part2(self):
        actual = solve_lines(puzzle_data)
        self.assertEqual(978171, actual)
        # >43049
